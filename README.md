...
동명의 SoftwareX 논문에서 평가한 v1.0 공식 배포본. 아래 그림과 표는 모두 논문에 수록된 것이다.

#### 표 1. 코드 메타데이터 (Code metadata)

| Nr | 항목 | 내용 |
|:---:|---|---|
| C1 | 현재 코드 버전 | FORECAST-SW v1.0 |
| C2 | 코드 저장소 영구 링크 | https://github.com/ISW-LAB/-SEJONG-SW/tree/v1.0 |
...
| C8 | 개발자 문서·매뉴얼 | English: [README.en.md](README.en.md); Korean: [README.md](README.md) |
...
> [!IMPORTANT]
> **적용 범위.** 상대생장식은 직경–바이오매스 관계를 정의하고, 직경 발달은 연간 생장량으로 따로 지정한다. 따라서 궤적은 **지정된 생장 가정 하의 결정론적 산출**이며 검증된 예측이 아니다. 적합 직경범위를 벗어난 값은 **외삽**이다. 식재면적 기준값과 3D 형상은 소프트웨어 설정값이지 생태적 수용력이나 실측 수관 구조가 아니다.
...
| 단계 | 담당 | 핵심 내용 |
|:---:|---|---|
| **1** | Library Manager | 4개 컬렉션에 식과 파라미터 정의 — 계수, 탄소계수, 적합범위, 기간별 생장량 3개(1–10 / 11–20 / 21–50년) |
...
| **7** | Assessment App | 저장된 생장량으로 0–50년 시나리오 투영, 현재 저장량과 **동일한 엔진**으로 평가 |
...
**77개 레코드 · 학명 67종.** 원 연구의 지역·임분 조건·바이오매스 구성요소가 다르면 같은 수종이라도 레코드를 분리해 보존한다 — 예: *Pinus thunbergii*(곰솔)는 **4개 레코드**.

| 컬렉션 | 레코드 | 예측변수 | 저장 형식 | 대상지 평가 | 시나리오 |
|---|---:|---|---|:---:|:---:|
| 교목 `TREE_BASE` | **7** | DBH (cm) | 계수 `a, b, CF` + 범위 + 생장량 | ✅ | ✅ |
| 관목 `SHRUB_SPECIES` | **15** | RCD (mm 적합) | 계수 `a, b, CF` + 범위 + 생장량 | ✅ | ✅ |
| 국내 `DOMESTIC_SPECIES` | **30** | DBH (+ 수고·밀도·LAI) | 수식 문자열 + 범위 | — | — |
| 국외 `FOREIGN_SPECIES` | **25** | DBH (+ 수고·밀도·LAI) | 수식 문자열 + 범위 | — | — |
| **합계** | **77** | | | **22** | **22** |

#### 표 2. FORECAST-SW 교목·관목 컬렉션의 대표 상대생장식 레코드 — 예측변수 정의, 적합 직경범위, 기간별 생장량

| 학명 | 상대생장식 | 예측변수 | 적합범위 | 1–10 | 11–20 | 21–50 |
|---|---|:---:|:---:|---:|---:|---:|
| ***교목 컬렉션*** | | | | | | |
...
- 마지막 세 열은 연간 직경 생장량(cm yr⁻¹). **0 = 해당 기간에 직경 생장을 두지 않음.**
- `X` 는 교목 DBH, 관목 RCD (cm). mm로 적합된 관목식은 **`10X`** 로 쓰고 적합 한계를 cm로 환산 — 원 관계식을 재적합 없이 보존한다.
- 대상지 유형은 **설명용 메타데이터**이며 계수를 선택하거나 변경하지 않는다 (`test_site_category_does_not_select_coefficients`).
...
- 국내·국외 레코드는 `Y_i = f_i(X_i, H_i)` 형태로 AST 허용목록을 통해 평가한다(Python `eval` 미사용). 탄소계수·생장량을 저장하지 않으므로 **고정 CF₀ = 0.5** 를 적용하고 시나리오에서 제외된다.
...
Manager 창은 4개 컬렉션과 함께 계수·탄소계수·적합 한계·생장률을 각각의 열로 노출하는 편집 가능한 레코드 표를 제시한다.
...
**(a)** 주 작업화면: 인벤토리 입력과 현재 교목·관목·총 저장량 · **(b)** 식 대화상자: 선택된 식, 계수, 탄소계수, 적합범위, 생장량 · **(c)** 0–50년 궤적과 수종별 기여도 · **(d)** 3D 임분 배치.

### 4.3 면적을 고려한 대상지 간 비교

**인벤토리 구성**에 따른 변동과 **면적 정규화**로 생기는 변동을 분리하도록 설계했다.

#### 표 3. Figure 4의 대상지 간 비교에 사용한 통제 인벤토리와 산출값
...
> **Figure 5.** 0년·20년·50년 시점의 교목·관목·총 탄소저장량과 해당 3D 임분 시각화를 보여주는 프로파일 1의 결정론적 성장 시나리오.
...
후반 연차 값에는 투영 직경이 적합범위를 벗어난 **외삽 추정치**가 포함될 수 있다. 3D 뷰는 소프트웨어가 정의한 시나리오 형상이며 실측 수관 구조가 아니다.

### 4.5 2단계 레코드 검증과 식재면적 안전장치

#### 표 4. 직경범위 검증과 식재면적 안전장치의 시험 사례 및 결과
...
Manager는 `species_data.json` 을 4개 탭의 편집 가능한 표로 연다(Figure 2). 저장 전 검증과 `.bak` 자동 백업을 지원하며, 핵심 소스 전체를 번들하므로 단독 실행된다.
...
회귀검사 22건이 표 3–4와 Figure 4–5의 값을 그대로 재현한다 — 라이브러리 레코드 수, 면적 정규화 밀도(`0.4972 / 0.3978 / 0.3315`), 개체수 비례성, 직경 경계값, 시나리오 반복성 및 0년 일치, 관목 mm↔cm 등가성, 호환 수식 55개 전체 실행, 위험 문법 차단. 동일 검사가 Windows · Python 3.10/3.11 CI에서 실행된다. 빌드 환경과 체크섬: [BUILD_VERIFICATION.md](BUILD_VERIFICATION.md).
...
├── tests/test_core.py             ← 회귀검사 22건 — 표 3–4
└── figures/paper/                 ← 논문 게재 Figure 1–6
```
...
<sub>본 연구는 농림축산식품부 IPET(RS-2024-00398561), 과학기술정보통신부 IITP(IITP-2026-RS-2020-II201462), 교육부 NRF(RS-2025-25430681)의 지원으로 수행되었습니다.</sub>
