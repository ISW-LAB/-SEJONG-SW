...
버전 4.4.1의 과학 라이브러리는 사용자 인터페이스에서 직접 사용하는 범위와 호환성 범위를
...
4. 산출물: `installer_output\FOCA-SW_Setup_4.4.1.exe`
...
빌드하거나 수식 라이브러리를 수정하기 전에 다음 회귀검사를 실행한다.

```powershell
python -m unittest discover -s tests -v
```

검사는 라이브러리 레코드 수, JSON 및 데이터 라이선스, 대상지 유형에 대한 계수 불변성,
상대생장식 계산, 개체수 비례성, 직경 경계값, 시나리오 반복성 및 0년 일치, 관목 단위 변환,
전체 호환 수식 실행, 허용되지 않은 수식 구문 차단을 확인한다. 동일한 검사는 저장소의
지속적 통합 절차에서 Windows와 Python 3.10 및 3.11 조합으로 실행된다. 평가에 사용한
Windows 빌드 환경과 로컬 산출물 체크섬은 [BUILD_VERIFICATION.md](BUILD_VERIFICATION.md)에
기록하였다.
...
소스 코드, 빌드 스크립트, 문서 및 저장소 그림은 [MIT License](LICENSE)로 배포한다.
`species_data.json`의 과학 수식 라이브러리는 공공누리 제1유형(출처표시)을 적용하며,
자세한 범위와 출처표시 방법은 [DATA_LICENSE.md](DATA_LICENSE.md)를 참고한다. 개별
상대생장식은 원 출처 문헌의 서지정보도 함께 유지해야 한다.
